    ...
    <q-modal v-model="formMdl" class="form-modal" :content-css="{minWidth: '50vw', minHeight: '45vh'}">
      <q-modal-layout>
        <q-toolbar slot="header">
          ...
          <q-toolbar-title v-if="!form.title">Add new Stock Origin</q-toolbar-title>
        </q-toolbar>
        <div class="body">
          <form :model="form" @submit.prevent="formSave()">
            <div class="row-input-header">
              <q-input float-label="Origin Name" v-model="form.origin"/>
            </div>
          </form>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      currView : 'list',
      listView :  {
                    pagination: {sortBy:'origin',descending:false,rowsPerPage: 25},
                    columns   : [
                                  {name:'origin',field:'origin',label:'Item Origin',align:'left',sortable:true},
  ...
  methods: {
    ...
    formSave(id){
          let url = 'stock_origins/store';
          if(id){
              url = 'stock_origins/update/'+id;
          }
        ...
        //console.log(this.form);
          ...
          if(data.status == 'success'){
              ...
              noty.message = 'Stock origin successfully saved';
              this.$q.notify(noty);
    ...
    formDelete(del){
          this.$q.dialog({
              ...
              this.$q.loading.show({messageColor: 'primary',spinnerSize: 250,spinnerColor: 'primary'});
          this.$api.post('stock_origins/delete/'+del.id)
              .then((response) => {
              let data = response.data.details;
          if(data){
                let noty = this.$noty.success;
                noty.message = 'Stock origin was successfully deleted';
                this.$q.notify(noty);
                this.loadList();
            }
            this.$q.loading.hide();
